...
xtest = np.array([[-1.37333962e-03,  3.48250919e-05],
...
for i in range(len(xtest[:, 0])):
       if not any(np.isnan(xtest[i, :])):
              if not any(np.isinf(xtest[i, :])):
                     xplot_.append(xtest[i, :])
print('xplot_: ', np.asarray(xplot_))
...
# print('')
# for i, j in edges:
#        # plt.plot(xplot[[i, j], 0], xplot[[i, j], 1])  # same as below
#        plt.plot([xplot[i, 0], xplot[j, 0]], [xplot[i, 1], xplot[j, 1]])
#
